        ...
        stage('Deploy Gateway') {
            ...
            steps {
                sshagent(credentials: ['esp31-deploy']){
                    ...
                    sh "ssh -o 'StrictHostKeyChecking=no' -l esp31 192.168.160.103 'echo 'docker run -p 3181:8081 192.168.160.99:5000/esp31-gateway' | at now'"
                }
            }
        }
        stage('Deploy Webserver') {
            ...
            steps {
                sshagent(credentials: ['esp31-deploy']){
                    ...
                    sh "ssh -o 'StrictHostKeyChecking=no' -l esp31 192.168.160.103 'echo 'docker run -p 3180:8080 192.168.160.99:5000/esp31-webserver' | at now'"
                }
            }
        }
        stage('Deploy Frontend') {
            ...
            steps {
                sshagent(credentials: ['esp31-deploy']){
                    ...
                    sh "ssh -o 'StrictHostKeyChecking=no' -l esp31 192.168.160.103 'echo 'docker run -p 3100:3000 192.168.160.99:5000/esp31-frontend' | at now'"
                }
            }
        }
    }
