    ...
    stages {
        ...
        stage('Publish Gateway') {
            steps {
				dir('gateway') {
                   sh '''
                        docker tag esp31-gateway 192.168.160.99:3100/esp31-gateway
                        docker push 192.168.160.99:3100/esp31-gateway
                    '''
                }
            }
        }
        stage('Publish Webserver') {
            steps {
				dir('webserver') {
                   sh '''
                        docker tag esp31-webserver 192.168.160.99:3100/esp31-webserver
                        docker push 192.168.160.99:3100/esp31-webserver
                    '''
                }
            }
        }
        stage('Publish Frontend') {
            steps {
				dir('frontend') {
                   sh '''
                        docker tag esp31-frontend 192.168.160.99:3100/esp31-frontend
                        docker push 192.168.160.99:3100/esp31-frontend
                    '''
                }
            }
        }
